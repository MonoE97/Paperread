...
def test_refresh_live_notes_command_updates_details(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("paper_reader.cli.fetch_item_children_notes", fake_fetch_item_children_notes)
    runner = CliRunner()
    ...
    assert payload["_paper_reader"]["enrichment"]["live_notes"]["status"] == "refreshed"
...
def test_verify_zotero_note_command_reports_pass(monkeypatch) -> None:
    ...
    monkeypatch.setattr("paper_reader.cli.fetch_note_snapshot", fake_fetch_note_snapshot)
    runner = CliRunner()
...
def test_prepare_write_candidate_command_writes_payload(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("paper_reader.cli.prepare_write_candidate", fake_prepare_write_candidate)
    result = CliRunner().invoke(
        app,
        [
