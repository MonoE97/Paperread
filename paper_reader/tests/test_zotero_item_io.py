...
from paper_reader.zotero_item_io import (
...
def test_write_item_details_files_enriches_missing_extra_from_sqlite(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("paper_reader.zotero_item_io.lookup_extra_by_item_key", fake_lookup, raising=False)
    ...
    assert normalized["_paper_reader"]["warnings"] == []
    assert normalized["_paper_reader"]["enrichment"]["extra"]["source"] == "zotero_sqlite"
    assert normalized["_paper_reader"]["enrichment"]["extra"]["diagnostics"] == ["sqlite_immutable_snapshot_used"]
...
def test_write_item_details_files_keeps_mcp_extra_without_sqlite_lookup(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("paper_reader.zotero_item_io.lookup_extra_by_item_key", fail_lookup, raising=False)
