...
paper_reader 是面向 Codex 或 Claude 的自包含 skill repo。它把 Zotero 论文标题、本地 PDF path 或多篇论文批量输入，转成有证据链的中文结构化阅读笔记；实现方式是确定性的 CLI 工具加 agent 写出的结构化总结。
...
安装时把对应 source 目录复制到目标 skill 目录，并在安装后的 skill root 中运行命令。CLI 负责准备确定性 artifacts、校验 gate、渲染笔记和记录 batch 状态；agent 仍然负责阅读抽取出的 context，并写出 `summary.json` / `review.json`。
...
`paper_reader` 用于一次分析一篇论文。CLI 是确定性工具，不是 standalone summarizer：它负责准备抽取产物并校验笔记是否 ready，agent 负责阅读生成的 context 文件并写出 `summary.json` 和 `review.json`。

- Zotero 标题或标题片段：让 agent 使用 `$paper_reader` 和论文标题。agent 会通过 Zotero MCP 搜索文献库，准备 evidence artifacts，写出结构化 summary 和 review，渲染 `note.md` 与 `note.html`，预览候选笔记，只在明确写入意图后通过 MCP `write_note` 写入，并回读校验新建笔记。
- 本地 PDF path：给出绝对或相对 `.pdf` 路径。`prepare-pdf` 会在 PDF 同目录创建 `<pdf_stem>_analysis/`，并预留 `<pdf_stem>_note.md` 作为目标；最终 Markdown 笔记只有在 agent 写出 `summary.json` / `review.json` 并运行 `prepare-local-note-candidate` 后才会生成。该 workflow 不搜索 Zotero 中是否有相同文献，也不会写 Zotero。
...
重复执行 `next -> worker-prompt -> record-result`，直到没有 pending item。`worker-prompt` 是交给外层 agent 或 subagent 的 handoff；batch CLI 本身不读论文。
...
- **本地 PDF path**：先对本地 PDF 运行抽取，由 agent 基于生成的 context 写 `summary.json` 与 `review.json`，再运行 review、lint、trusted-summary validation 和本地渲染门禁。最终 Markdown 笔记写到 PDF 同目录，不写入 Zotero，也不检查 Zotero 中是否已有同一篇论文。
...
- 本地 PDF path 工作流的产物默认写在 PDF 同目录：`prepare-pdf` 创建 `<pdf_stem>_analysis/` 保存分析产物，并预留 `<pdf_stem>_note.md`；`prepare-local-note-candidate` 在 local gate 通过后写出最终 Markdown 笔记。已有输出不会覆盖，会自动使用 `_v2`、`_v3` 等后缀。
