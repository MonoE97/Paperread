...
- Use `paper_reader_batch`: 多篇论文调度。batch skill 负责 manifest/state/report、`next`/`record-result`、`next-write`/`record-write`；每篇仍派发给 `$paper_reader`，PDF folder/path items 保持 local-output only 且不做 Zotero lookup / duplicate check。
