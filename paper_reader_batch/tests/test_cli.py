...
from paper_reader_batch import cli as cli_module
from paper_reader_batch.cli import app
...
def _fake_paper_reader_root(tmp_path: Path, *, include_pyproject: bool = True) -> Path:
    root = tmp_path / "paper_reader"
    root.mkdir()
...
def _successful_result(tmp_path: Path, item_id: str = "001", *, write_ready: bool = False) -> Path:
    run_dir = tmp_path / f"paper_reader-run-{item_id}"
    run_dir.mkdir()
...
def test_batch_run_init_validate_next_record_report_retry(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_run(command, cwd, **_kwargs):
        assert command == ["uv", "run", "paper_reader", "--help"]
        assert Path(cwd) == paper_reader_root
        return SimpleNamespace(returncode=0, stderr="")

    monkeypatch.setattr(cli_module.subprocess, "run", fake_run)
    result = runner.invoke(app, ["validate", str(batch_run), "--paper-reader-root", str(paper_reader_root)])
    assert result.exit_code == 0, result.output
...
def test_prepare_local_pdfs_records_prepared_pdf_bundle(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_prepare_pdf(*, paper_reader_root, pdf_path, timeout_seconds):
    ...
    result = runner.invoke(
        app,
        ["prepare-local-pdfs", str(batch_run), "--paper-reader-root", str(paper_reader_root), "--concurrency", "2"],
    )
    ...
    assert "Use figure_context.md only if it is present" in result.output
    assert "Do not run prepare-pdf again" in result.output


def test_prepare_local_pdfs_skips_non_pending_items(tmp_path: Path, monkeypatch) -> None:
    pdf = tmp_path / "paper.pdf"
    pdf.write_bytes(b"%PDF-1.4\n")
    paths = tmp_path / "paths.txt"
    paths.write_text(str(pdf), encoding="utf-8")
    ...
    state["items"][0]["local_prepare_failure_reason"] = "previous failure"
    (batch_run / "state.json").write_text(json.dumps(state), encoding="utf-8")
    ...
    result = runner.invoke(
        app,
        ["prepare-local-pdfs", str(batch_run), "--paper-reader-root", str(paper_reader_root), "--concurrency", "1"],
    )
    ...
    assert state["items"][0]["local_prepare_failure_reason"] == "previous failure"
...
def test_init_without_output_allocates_default_run_dir(tmp_path: Path, monkeypatch) -> None:
    batch_root = tmp_path / "paper_reader_batch"
    batch_root.mkdir()
    monkeypatch.setattr(cli_module, "_paper_reader_batch_root", lambda: batch_root)
    titles = tmp_path / "titles.txt"
    titles.write_text("First paper\n", encoding="utf-8")
    manifest_path = tmp_path / "manifest.json"
...
def test_validate_rejects_invalid_explicit_paper_reader_root_without_fallback(tmp_path: Path, monkeypatch) -> None:
    titles = tmp_path / "titles.txt"
    titles.write_text("First paper\n", encoding="utf-8")
    manifest_path = tmp_path / "manifest.json"
    batch_run = tmp_path / "batch-run"
    batch_root = tmp_path / "paper_reader_batch"
    batch_root.mkdir()
    fallback_root = tmp_path / "paper_reader"
    fallback_root.mkdir()
    (fallback_root / "SKILL.md").write_text(
        "---\nname: paper_reader\ndescription: fallback.\n---\n# paper_reader\n",
        encoding="utf-8",
    )
    explicit_root = tmp_path / "not-paper_reader"
    explicit_root.mkdir()
    monkeypatch.setattr(cli_module, "_paper_reader_batch_root", lambda: batch_root)
    ...
    result = runner.invoke(app, ["validate", str(batch_run), "--paper-reader-root", str(explicit_root)])

    assert result.exit_code == 1
    assert "paper_reader_unavailable" in result.output
    assert str(explicit_root) in result.output


def test_validate_rejects_paper_reader_root_without_runnable_cli(tmp_path: Path) -> None:
    titles = tmp_path / "titles.txt"
    titles.write_text("First paper\n", encoding="utf-8")
    manifest_path = tmp_path / "manifest.json"
    batch_run = tmp_path / "batch-run"
    paper_reader_root = _fake_paper_reader_root(tmp_path, include_pyproject=False)
