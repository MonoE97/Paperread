...
from paper_reader_batch.io import file_sha256, write_json_atomic
from paper_reader_batch.manifest import PREPARE_ONLY_WRITE_POLICY, build_manifest, validate_manifest
...
def _write_ready_result(tmp_path: Path, *, item_id: str = "001", worker_id: str = "worker-001") -> dict:
    run_dir = tmp_path / "paper_reader" / "runs" / f"paper-{item_id}"
...
def test_record_success_requires_zotero_evidence_and_updates_state(tmp_path: Path) -> None:
    ...
    run_dir = tmp_path / "paper_reader" / "runs" / "paper"
    result = {
        "schema_version": "paper_reader_batch.item-result.v1",
        "item_id": "001",
    ...
    assert item["paper_reader_run_dir"] == str(run_dir)
...
def test_record_success_rejects_write_payload_when_gate_is_not_write_ready(tmp_path: Path) -> None:
    ...
    result["gate_report"] = _write_json(Path(result["paper_reader_run_dir"]) / "gate-report.json", {"status": "blocked"})
...
def test_record_write_result_requires_passed_verify_report(tmp_path: Path) -> None:
    ...
    verify_report = _write_json(
        tmp_path / "paper_reader" / "runs" / "paper-001" / "verify-report.json",
        {
            "status": "passed",
            "noteKey": "NOTE1",
            "parentKey": "PARENT1",
...
def test_record_result_rejects_mismatched_item_id() -> None:
    ...
    with pytest.raises(StateError, match="mismatched item_id"):
        record_item_result(
            ...
            manifest,
            "001",
            {"schema_version": "paper_reader_batch.item-result.v1", "item_id": "002", "status": "failed"},
            now="2026-07-02T10:02:00+08:00",
        )
