...
from paper_reader_batch.io import JsonFileError, exclusive_file_lock, read_json, write_json_atomic, write_text_atomic
from paper_reader_batch.manifest import (
...
from paper_reader_batch.runs import allocate_batch_run_dir
from paper_reader_batch.state import (
    INTERRUPTED,
    PENDING,
    RUNNING,
    StateError,
...
from paper_reader_batch.worker_contract import render_worker_prompt

console = Console()
app = typer.Typer(help="Batch orchestration utilities for paper_reader.")
...
def _paper_reader_batch_root() -> Path:
    return Path(__file__).resolve().parents[2]


def _find_paper_reader_root(explicit_root: Path | None) -> Path | None:
    ...
    env_root = os.environ.get("PAPER_READER_SKILL_ROOT")
    if env_root:
        candidates.append(Path(env_root))
    batch_root = _paper_reader_batch_root()
    candidates.append(batch_root.parent / "paper_reader")
...
def _validate_paper_reader_root(explicit_root: Path | None) -> Path:
    root = _find_paper_reader_root(explicit_root)
    if root is None:
        if explicit_root is not None:
            _exit_error(f"paper_reader_unavailable: --paper-reader-root does not contain SKILL.md: {explicit_root}")
        _exit_error("paper_reader_unavailable: set --paper-reader-root or PAPER_READER_SKILL_ROOT")
    skill_md = root / "SKILL.md"
    text = skill_md.read_text(encoding="utf-8")
    if "\nname: paper_reader\n" not in text:
        _exit_error(f"paper_reader_invalid: {skill_md} does not declare name: paper_reader")
    if not (root / "pyproject.toml").exists():
        _exit_error(f"paper_reader_unavailable: missing pyproject.toml at {root}")
    try:
        ...
        _exit_error(f"paper_reader_unavailable: cannot invoke paper_reader at {root}: {exc}")
    if result.returncode != 0:
        _exit_error(f"paper_reader_unavailable: uv run paper_reader --help failed at {root}: {result.stderr}")
    return root
...
def validate_command(
    batch_run: Path,
    paper_reader_root: Path | None = typer.Option(None, "--paper-reader-root", help="Installed paper_reader skill root."),
) -> None:
    ...
    root = _validate_paper_reader_root(paper_reader_root)
    console.print(f"batch_run_valid: {run_dir}")
    console.print(f"paper_reader_root: {root}")
