...
def validate_skill(skill_root: Path) -> list[str]:
    ...
    if skill_md.exists():
        try:
            ...
            if metadata.get("name") != "paperread":
                errors.append("frontmatter name must be paperread")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the portable Paperread skill bundle.")
