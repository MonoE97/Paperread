...
- Local PDF path: use `$paperread` with a `.pdf` path. The skill prepares `<pdf_stem>_analysis/` and `<pdf_stem>_note.md` beside the PDF, never searches Zotero for matching items, and never writes Zotero.
- Local directory path: route to `$paperread-batch` with the local PDF folder workflow. Directory input is not a Zotero title fragment.
...
- If the user input resolves to an existing local directory path, delegate to `$paperread-batch` and its local PDF folder workflow. If `$paperread-batch` is unavailable, ask the user to install or enable it; do not fall back to Zotero title search.
